...
class MemoryLayer:
    ...
    def store(self, content: str, memory_type: str, metadata: dict[str, Any] | None = None, context: TenantContext | None = None) -> dict[str, Any]:
        ...
        record = {"id": digest, "tenant_id": context.tenant_id, "owner": context.subject, "content": content, "memory_type": memory_type, "metadata": metadata, "created_at": int(time.time()), "archived": False}
    ...
    def search(self, query: str, limit: int = 5, context: TenantContext | None = None) -> dict[str, Any]:
        ...
        return {"query": query, "tenant_id": context.tenant_id, "results": [{**record, "score": score} for score, record in ranked[:limit]]}

    def consolidate(self, context: TenantContext | None = None) -> dict[str, Any]:
        if context is None:
            raise ValueError("tenant context is required")
        by_type: dict[str, list[str]] = {}
        tenant_records = [record for record in self.records.values() if record.get("tenant_id") == context.tenant_id]
        ...
        return {"tenant_id": context.tenant_id, "summaries": summaries, "record_count": len(tenant_records)}
